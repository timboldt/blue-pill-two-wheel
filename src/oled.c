...
static void _WriteBytes(uint8_t *bytes, uint8_t len) {
...
static void _MoveToRowCol(uint8_t row, uint8_t col) {
  ...
  _WriteBytes(cmd, sizeof(cmd));
}

void OLED_Init() {
  uint8_t cmd[] = {// command mode
                   0x00,
                   // display off
                   SSD1306_DISPLAYOFF,
  ...
  //     status =
  //         HAL_I2C_Master_Transmit(&hi2c1, 0x3c << 1, data, sizeof(data),
  //         100);
  //   } while (status != HAL_OK);
  // }
